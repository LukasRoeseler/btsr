...
(() => {
  ...
  const I18N_EN = {
    ...
    "Rennen":
      "Race",
    "Ein Ghost fährt auf der Start/Ziel-Kachel rechts an den Rand, bleibt ein paar Sekunden stehen und fährt wieder los. Die Anfahrt beginnt schon auf der Kachel davor, im Formationstempo – ein Auto, das mit Renntempo über die Linie kommt, braucht eine Kachel zum Verzögern. Es pittet immer nur einer; wer gleichzeitig fällig ist, wartet, bis die Box frei ist – nicht genau eine Runde. Gemessen in der Rennsimulation ist die Box mit den Vorgabewerten 28 % der Zeit belegt (vier Ghosts) bis 31 % (sechs). Mit einem engen Intervall sättigt sie: bei 2 bis 4 Runden und sechs Ghosts waren es 70 %, und dann kommt jeder später dran als er fällig wäre.":
      "A ghost pulls over to the right-hand edge on the start/finish tile, stands there for a few seconds and drives off again. The approach begins on the tile before it, at formation pace – a car crossing the line at racing speed needs a tile to slow down. Only one pits at a time; whoever is due at the same moment waits until the pit is free – not exactly one lap. Measured in the race simulation, the pit is occupied 28% of the time at the default settings (four ghosts) to 31% (six). A short interval saturates it: at 2 to 4 laps with six ghosts it was 70%, and then everyone gets their stop later than they are due.",
    "Ghost: Boxenstopp":
      "Ghost: pit stop",
