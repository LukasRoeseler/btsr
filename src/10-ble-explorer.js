...
(() => {
  ...
  const I18N_EN = {
    ...
    "Und ein Boxenstopp: der Knopf dafür steht auf dem Schirm von Auto 2, nicht neben dem von Auto 1 – zwei gleich aussehende Boxenknöpfe nebeneinander sind die Bedienung, bei der man im Fahren daneben greift. Anfordern, anhalten, warten, losfahren; nochmal drücken bricht ab. Es tankt mit denselben 22 %/s wie Auto 1 und repariert mit derselben nichtlinearen Rate – ungleiche Raten wären schlimmer als kein Stopp. Während des Stopps gilt dasselbe Tempolimit, aber als eigener Deckel: der von Auto 1 läuft über einen Weg, den Auto 2 gar nicht nimmt.":
      "And a pit stop: its button sits on car 2's own screen, not next to car 1's – two identical-looking pit buttons side by side are the kind of control you grab wrongly while driving. Request, stop, wait, drive off; pressing again aborts. It refuels at the same 22 %/s as car 1 and repairs at the same non-linear rate – unequal rates would be worse than no stop. The same speed limit applies during the stop, but as its own cap: car 1's runs through a path car 2 does not take.",
