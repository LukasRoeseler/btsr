...
(() => {
  ...
  const I18N_EN = {
    ...
    "Sportwagen-Gefühl statt Rennwagen: mehr Gewicht, weichere Bremse":
      "Sports car feel instead of a race car: more weight, softer brakes",
    ...
    "Alle sind aus Zylinderzahl, Kurbelwelle, Bankaufteilung und Zündfolge gerechnet – bis auf eine Ausnahme keine Aufnahme. Die Anlasser-Aufnahme des Porsche steht als eigenes Profil direkt neben dem synthetischen Original, zum Vergleich, nicht als dessen Ersatz. Bei den aufgeladenen Originalen fehlt der Lader. Die sechs mit WIP sind noch nicht nach Gehör geprüft; beim Maserati kommt dazu, dass dieses Modell den Bankwinkel gar nicht darstellt, weshalb er sich vom Ferrari-V12 nur in Drehzahl und Rohrlänge unterscheidet. Der Boxer-Rumpel des Subaru kommt aus ungleich langen Krümmerrohren; das Modell trägt feste Zeitversätze je Zylinder und damit die richtige Art von Unregelmäßigkeit, aber ihr genaues Muster ist gewählt und nicht aus Rohrlängen gerechnet.": "All of them are computed from cylinder count, crankshaft, bank split and firing order – with one exception, none is a recording. The Porsche's starter recording stands as its own profile right next to the synthetic original, for comparison, not as its replacement. The forced-induction originals are missing their turbo. The six marked WIP have not been judged by ear; with the Maserati there is the added point that this model cannot represent bank angle at all, so it differs from the Ferrari V12 only in revs and pipe length. The Subaru boxer rumble comes from unequal-length headers; the model carries fixed per-cylinder timing offsets and therefore the right KIND of irregularity, but their exact pattern is chosen, not computed from pipe lengths.",
